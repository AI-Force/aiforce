...
__all__ = ['BUF_SIZE', 'FILE_FILTER', 'IMAGE_EXTENSION', 'DoubleFileChecker', 'get_file_sha', 'check_double_entries',
           'remove_entries', 'scan_folder']
...
class DoubleFileChecker:
    ...
    def check(self):
        ...
        for index, image in enumerate(images):
            print(index + 1, " / ", all_images, " - Handle Image ", image)
            content = (image, get_file_sha(image))
            contents.append(content)

        # Check double entries
        print('Checking Double Entries:')
        double_entries = check_double_entries(contents)

        print('Found ', len(double_entries), ' Entries:')
...
def remove_entries(entries, to_remove, delete_source=False):
    ...
    for entry in to_remove:
        if entry in entries:
            ...
            print('Remove Entry: (', index, ') ', entry[0])
            del entries[index]
            if delete_source:
                print('Delete Source File: ', entry[0])
                remove(entry[0])
