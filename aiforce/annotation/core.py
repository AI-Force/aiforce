...
__all__ = ['logger', 'SubsetType', 'RegionShape', 'parse_region_shape', 'Region', 'Annotation', 'annotation_filter',
           'convert_region', 'region_bounding_box', 'AnnotationAdapter', 'normalize_value', 'denormalize_value']
...
class Region:
    ...
    def normalize(self, width: int, height: int):
        self.points_x = list(map(lambda x: normalize_value(x, width), self.points_x))
        self.points_y = list(map(lambda y: normalize_value(y, height), self.points_y))
        self.radius_x = normalize_value(self.radius_x, width)
        self.radius_y = normalize_value(self.radius_y, height)

    def denormalize(self, width, height):
        self.points_x = list(map(lambda x: denormalize_value(x, width), self.points_x))
        self.points_y = list(map(lambda y: denormalize_value(y, height), self.points_y))
        self.radius_x = denormalize_value(self.radius_x, width)
        self.radius_y = denormalize_value(self.radius_y, height)
